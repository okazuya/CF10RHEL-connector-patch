                                 ...
                                 jk_logger_t *l, ajp_endpoint_t * ae)
{
     ...
     */
    /* Start of CFChanges */
	if(s->server_software){
		if (jk_b_append_byte(msg, SC_A_SERVER_SOFTWARE) ||
            jk_b_append_string(msg, s->server_software)) {
            jk_log(l, JK_LOG_ERROR,
                   "failed appending the server software");
            JK_TRACE_EXIT(l);
            return JK_FALSE;
        }
	}
	if(s->gateway_interface){
		if (jk_b_append_byte(msg, SC_A_GATEWAY_INTERFACE) ||
            jk_b_append_string(msg, s->gateway_interface)) {
            jk_log(l, JK_LOG_ERROR,
                   "failed appending the gateway interface");
            JK_TRACE_EXIT(l);
            return JK_FALSE;
        }
	}
...
void ajp_close_endpoint(ajp_endpoint_t * ae, jk_logger_t *l)
{
    ...
    ae->sd = JK_INVALID_SOCKET;
	 /* Below code changes are incorporated by Adobe Systems Incorporated
 	  * Purpose : Support for reuse connections configration
	  */
	/* Start of CFChanges */
	ae->worker->current_reuse_connections--;
	/* End of CFChanges */
                                ...
                                jk_ws_service_t *r, jk_logger_t *l)
{
    ...
    case AJP13_GETREALPATH:
		 ...
		 {
          jk_log(l, JK_LOG_ERROR,
                  "uri in AJP13_GETREALPATH is null");
          JK_TRACE_EXIT(l);
          return JK_UNSET;
         }
         jk_log(l, JK_LOG_DEBUG,"Getting real path");

		 path=r->getRealPath(r,uri,l);
		 jk_b_reset(msg);
		 jk_b_reset(pmsg);
		 len = strlen(path);
/*
		 ...
		 if (jk_b_set_buffer_size(pmsg,size)) {
          jk_log(l, JK_LOG_ERROR,
               "Failed allocating AJP message buffer in AJP13_GETREALPATH");
          JK_TRACE_EXIT(l);
          return JK_UNSET;
         }
*/
pmsg->len = 0;
		 ...
		 if(jk_b_append_byte(pmsg,len)){
          jk_log(l, JK_LOG_ERROR,
               "Failed allocating len+1 bytes AJP13_GETREALPATH");
          JK_TRACE_EXIT(l);
          return JK_UNSET;
         }
		 if(jk_b_append_bytes(pmsg,path,len)){
          jk_log(l, JK_LOG_ERROR,
               "Failed allocating path bytes AJP13_GETREALPATH");
          JK_TRACE_EXIT(l);
          return JK_UNSET;
         }
		 free(path);

		 ae->last_op = AJP13_GETREALPATH;
		 
		 return AJP13_GETREALPATH;
         }
		break;         
	/* End of CFChanges */
    ...
    case JK_AJP13_END_RESPONSE:
	{
        ...
        if (!ae->reuse) {
            ...
            if(reuse == JK_FALSE)
			{
				if(ae->worker->max_reuse_connections == -1 || 
					(ae->worker->max_reuse_connections > ae->worker->current_reuse_connections))
				{
					ae->reuse = JK_TRUE;
					ae->worker->current_reuse_connections++;
					/* Reuse in all cases */
					if (JK_IS_DEBUG_LEVEL(l)) {
						jk_log(l, JK_LOG_DEBUG, "AJP13 protocol: Reuse is OK, and current reuse count is %u", ae->worker->current_reuse_connections);
					}
				}else
				{
					ae->reuse = JK_FALSE;
                         ...
                         ajp_endpoint_t * p, ajp_operation_t * op)
{
    ...
    while (1) {
		 ...
		 */
        /* Start of CFChanges */
        else if (JK_AJP13_HAS_RESPONSE == rc || AJP13_GETREALPATH == rc) {
		/* End of the CFChanges */
            /*
             ...
             jk_map_t *props, jk_worker_env_t *we, jk_logger_t *l, int proto)
{
    ...
    if (pThis && pThis->worker_private) {
		...
		/* Below code changes are incorporated by Adobe Systems Incorporated
         * Purpose : Support for reuse connection configuration 
         */
        /* Start of CFChanges */
		p->max_reuse_connections = 
			jk_get_reuse_count(props, p->name);
		/* End of CFChanges */
                ...
                for (i = (int)aw->ep_cache_sz - 1;
                    ...
                    if (aw->ep_cache[i] && IS_VALID_SOCKET(aw->ep_cache[i]->sd)) {
                        ...
                        if (elapsed > aw->cache_timeout) {
							 ...
							 */
							/* Start of CFChanges */
							if(aw->ep_cache[i]->reuse == JK_TRUE)
								aw->current_reuse_connections--;
							/* End of CFChanges */
